    ...
    <main className="p-8 max-w-2xl mx-auto font-sans">
      {!user ? (
        <div className="text-center py-20">
          <h1 className="text-3xl font-bold mb-6 text-gray-900">Bookmark Manager</h1>
          <button onClick={login} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition">
            Sign in with Google
          </button>
        </div>
      ) : (
        <div className="space-y-8">
          ...
          <form onSubmit={addBookmark} className="grid grid-cols-1 gap-4 p-4 border rounded-xl bg-gray-50">
            <input 
              ...
              className="p-2 border rounded text-gray-900 bg-white placeholder-gray-400"
            />
            <input 
          ...
          <div className="space-y-4">
            {bookmarks.map((bm) => (
              <div key={bm.id} className="flex justify-between items-center p-4 border rounded-lg hover:shadow-sm transition bg-white text-gray-900">
                <div>
                  <h3 className="font-semibold">{bm.title}</h3>
                  <a href={bm.url} target="_blank" rel="noreferrer" className="text-blue-500 text-sm truncate block max-w-xs">
                    {bm.url}
                  </a>
                </div>
                <button 
